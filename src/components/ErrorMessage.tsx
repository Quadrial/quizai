...
const ErrorMessage: React.FC<ErrorMessageProps> = ({ message, onRetry, onDismiss }) => {
  return (
    <div className="qa-alert qa-alert--error" role="alert" aria-live="polite">
      <div className="qa-alert__row">
        <div className="qa-alert__icon" aria-hidden="true">
          <HiExclamationTriangle className="qa-ico qa-ico--btn" />
        </div>

        <div style={{ flex: 1, minWidth: 0 }}>
          <div className="qa-alert__title">Error Occurred</div>
          <p className="qa-alert__message">{message}</p>

          {(onRetry || onDismiss) && (
            <div className="qa-alert__actions">
              {onRetry && (
                <button onClick={onRetry} className="qa-btn qa-btn--danger">
                  Try Again
                </button>
              )}
              {onDismiss && (
                <button onClick={onDismiss} className="qa-btn qa-btn--dangerSoft">
                  Dismiss
                </button>
              )}
            </div>
